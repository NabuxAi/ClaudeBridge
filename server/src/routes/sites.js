...
// Load the site (owned by the signed-in user) or 404.
function loadSite(req, res) {
  const raw = sites.rawForUser(req.params.id, req.user.sub)
...
function concern(name) {
  return async (req, res) => {
    ...
    if (name === 'settings') {
      const c = site.connector ? safeJson(site.connector) : null
...
router.post('/sites/:id/ping', async (req, res) => {
  ...
  try {
    ...
    sites.markPaired(site.id, { version: out.version })
