...
// dist/ is generated by scripts/build-*.sh and is gitignored, so a fresh
// checkout does not have it. Assertions about the built artifacts skip there
// rather than failing with ENOENT — a wall of red about a missing directory
// teaches people to ignore this file. CI builds first, so they never skip
// where it matters, which is the run that gates a release.
const BUILDS = [
  'dist/digiwp-ai-bridge/digiwp-ai-bridge.php',
  'dist/digi-ai-bridge/digi-ai-bridge.php',
]
const built = BUILDS.every((rel) => existsSync(join(root, rel)))
const needsBuild = built ? undefined : 'run scripts/build-digiwp-ai-bridge.sh and build-wporg-bridge.sh first'
...
test('the built distributables carry the same version', { skip: needsBuild }, () => {
...
test('the security_scan fix is present in every shipped build', { skip: needsBuild }, () => {
...
test('site_info reports the bridge version', { skip: needsBuild }, () => {
...
test('job_start advertises every job type the plugin can actually run', { skip: needsBuild }, () => {
